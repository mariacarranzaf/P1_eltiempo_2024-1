...
const mytestconfig = {
  server_url: "https://api.openweathermap.org/data/2.5/forecast/daily",
...
test(JSON.stringify(testinfo), async () => {
  render(<Resultados numitems={6} items={mock3} />);
  const resultado = document.querySelector('#resultados');
  expect(resultado).toBeInTheDocument();  
  expect(resultado).toHaveTextContent(/El tiempo/i);
  expect(resultado).toHaveTextContent(/Kuito/);
  expect(resultado).toHaveTextContent(/3600/);
  expect(resultado).toHaveTextContent("25/8/2024");
  expect(resultado).toHaveTextContent("28/8/2024");
...
test(JSON.stringify(testinfo), async () => {
  ...
  expect(resultado).toBeInTheDocument();
  expect(resultado).toHaveTextContent(/El tiempo/i);
  expect(resultado).toHaveTextContent(/Longcang/);
  expect(resultado).toHaveTextContent(/28800/);
  expect(resultado).toHaveTextContent(/China/);
  expect(resultado).toHaveTextContent("25/8/2024");
  expect(resultado).toHaveTextContent("30/8/2024");
  const imagenes = document.querySelectorAll('.tiempoimg');
